...
function initMap1(res) {
    ...
    let lokasi_pembeli = res[0];
    let latitude_pb = parseFloat(lokasi_pembeli.latitude), longitude_pb = parseFloat(lokasi_pembeli.longitude);
    let point = new google.maps.LatLng(latitude_pb, longitude_pb);
    const titik_saat_ini = new google.maps.LatLng(latitude_pb, longitude_pb);
    ...
    // MARKER
    marker = new google.maps.Marker({ //untuk nampilin marker
        map: map,
        position: point,
        zoom: 11
    });
    ...
    //origin -7.563823, 110.834977
    //tes origin 2 -7.571133, 110.806309
    var origin1 = { lat: parseFloat(latUsaha), lng: parseFloat(lngUsaha) };
        ...
        if (status !== 'OK') {
            ...
            for (var i = 0; i < originList.length; i++) {
                ...
                for (var j = 0; j < results.length; j++) {
                    ...
                    let distance_km = results[j].distance.text;
                    document.getElementById("output").innerHTML = distance_km;
...
function loadPesanan() {
    ...
    $.each(keranjangku, function (key, val) {
        console.log(val);
        var harga_total_per_produk = val.total_harga;
        ...
        produk_saya = '<li class="collection-item avatar" id="item-produk-' + key + '">' +
            ...
            '<p><span class="harga-produk orange-text">Rp ' + val.harga_produk + '</span>' + '<span class="black-text">&nbsp &times &nbsp' + val.qty + '&nbsp Kg' + '</span></p>' +
...
function TotalPembayaran(feeKirim_temp, biayaProduk) {
...
function Beli() {
    ...
    var DataPemesanan = {
        ...
        jpengiriman: Jpengiriman,
        jpembayaran: Jpembayaran,
